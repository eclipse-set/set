...
import { IUebersichtplanFeature } from '@/feature/UebersichtplanFeature'
...
export default class FeatureService extends Vue {
  ...
  listFeature: IUebersichtplanFeature[] = []
  inLODView = false
  ...
  private loadModel () {
    ...
    axios
      .get<SiteplanModel>('/overviewplan.json')
      .then(response => {
  ...
  private loadFeatureType (
    model: SiteplanModel,
    featureClass: IUebersichtplanFeature
  ): Feature<Geometry>[] {
    try {
