...
abstract class AbstractTableModelTransformator<T> implements TableModelTransformator<T> {
		...
		try {
			...
			new Thread([
				try {
					...
					val result = filling.apply(object)
					val changeProperty = new Pt1TableChangeProperties(
						object.container, row, column, #[result],
						ITERABLE_FILLING_SEPARATOR)
	...
	def <S, T extends Ur_Objekt> void fillDelayMultiCell(TableRow row,
		ColumnDescriptor column,
		T object,
		()=>Boolean shouldFill,
		String tableShortcut,
