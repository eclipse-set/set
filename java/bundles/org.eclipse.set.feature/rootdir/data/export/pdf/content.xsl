...
<!-- Copyright (c) 2017 DB Netz AG and others. All rights reserved. This 
...
<xsl:stylesheet version="2.0"
	...
	<xsl:template
		match="StringContent[starts-with(.,'Error:') or starts-with(.,'&#9203;')]"
		priority="1">
		<fo:block hyphenate="true" language="de">
			<fo:external-graphic
				xsl:use-attribute-sets="external-graphic-style"
				src="data/pictures/warning_black.svg" fox:alt-text="Error" />
		</fo:block>
	</xsl:template>
		...
		<fo:block keep-together.within-page="always">
			<xsl:choose>
				<xsl:when test="@keep-inline='true'">
					...
					<xsl:for-each select="*[@isFootNote='true']">
						<xsl:apply-templates select="." />
	...
	<xsl:template
		match="SimpleValue[starts-with(.,'Error:') or starts-with(.,'&#9203;')]">
		<fo:block hyphenate="true" language="de">
			<fo:external-graphic
				xsl:use-attribute-sets="external-graphic-style"
				src="data/pictures/warning_black.svg" fox:alt-text="Error" />
	...
	<xsl:template
		match="MultiColorValue[starts-with(.,'Error:') or starts-with(.,'&#9203;')]">
		<fo:block hyphenate="true" language="de">
			<fo:external-graphic
				xsl:use-attribute-sets="external-graphic-style"
				src="data/pictures/warning_black.svg" fox:alt-text="Error" />
