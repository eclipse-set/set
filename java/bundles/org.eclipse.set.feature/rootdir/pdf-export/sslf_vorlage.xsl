	...
	<xsl:template match="Table[Rows/Row]">
		<fo:table table-layout="fixed" width="100%">
			...
			<!-- CUSTOMIZE: Adjust number of used columns and insert column width 
				from excel template. -->
			<fo:table-column column-number="1" column-width="0.56cm" /> <!-- group number -->
			<fo:table-column column-number="2" column-width="1.8cm" /> <!-- A -->
			<fo:table-column column-number="3" column-width="1.48cm" /> <!-- B -->
			<fo:table-column column-number="4" column-width="1.69cm" /> <!-- C -->
			<fo:table-column column-number="5" column-width="0.77cm" /> <!-- D -->
			<fo:table-column column-number="6" column-width="1.56cm" /> <!-- E -->
			<fo:table-column column-number="7" column-width="1.69cm" /> <!-- F -->
			<fo:table-column column-number="8" column-width="1.64cm" /> <!-- G -->
			<fo:table-column column-number="9" column-width="1.69cm" /> <!-- H -->
			<fo:table-column column-number="10" column-width="1.3cm" /> <!-- I -->
			<fo:table-column column-number="11" column-width="1.69cm" /> <!-- J -->
			<fo:table-column column-number="12" column-width="1.3cm" /> <!-- K -->
			<fo:table-column column-number="13" column-width="1.59cm" /> <!-- L -->
			<fo:table-column column-number="14" column-width="1.46cm" /> <!-- M -->
			<fo:table-column column-number="15" column-width="1.52cm" /> <!-- N -->
			<fo:table-column column-number="16" column-width="17.24cm" /> <!-- O -->
			...
			<fo:table-header xsl:use-attribute-sets="table-header-style">
				...
				<fo:table-row font-weight="bold" display-align="before">
					...
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						border-right="{$wide-border-style}" border-top="{$wide-border-style}"
						number-columns-spanned="2">
						<fo:block>Flankenschutzanforderer</fo:block>
					</fo:table-cell>
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						border-right="{$wide-border-style}" border-top="{$wide-border-style}"
						number-columns-spanned="5">
						<fo:block>Unmittelbarer Flankenschutz</fo:block>
					</fo:table-cell>
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						border-top="{$wide-border-style}" border-right="{$wide-border-style}"
						number-columns-spanned="4">
						<fo:block>Weitergabe</fo:block>
					</fo:table-cell>
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						border-top="{$wide-border-style}" border-right="{$wide-border-style}"
				...
				<fo:table-row>
					...
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						...
						<fo:block>Bezeichnung Weiche/Nb</fo:block>
					</fo:table-cell>
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						border-right="{$wide-border-style}" number-rows-spanned="2">
						<fo:block>W-Lage/ Nb-Grenze</fo:block>
					</fo:table-cell>
					<fo:table-cell number-columns-spanned="3"
						xsl:use-attribute-sets="thin-border-style">
						<fo:block>Weiche/Gleissperre</fo:block>
					</fo:table-cell>
					<fo:table-cell number-columns-spanned="2"
						xsl:use-attribute-sets="thin-border-style" border-right="{$wide-border-style}">
					...
					<fo:table-cell number-rows-spanned="2"
						xsl:use-attribute-sets="thin-border-style" border-right="{$wide-border-style}">
						<fo:block>nicht freigemeldet</fo:block>
					</fo:table-cell>
				</fo:table-row>
				<fo:table-row>
					<fo:table-cell xsl:use-attribute-sets="no-border-style"
					...
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						border-right="{$wide-border-style}" number-rows-spanned="1">
						<fo:block>Rangierziel- sperre</fo:block>
					</fo:table-cell>
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						number-rows-spanned="1">
						<fo:block>Bezeichnung</fo:block>
...
über</fo:block>
				...
				<fo:table-row>
					...
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						border-bottom="{$wide-border-style}" border-right="{$wide-border-style}">
						<fo:block></fo:block>
					</fo:table-cell>
					<fo:table-cell xsl:use-attribute-sets="thin-border-style"
						border-bottom="{$wide-border-style}">
	...
	<xsl:template match="Cell[contains(' 2 7 11 12 14 ', concat(' ', @column-number, ' '))]" priority="1">
		<fo:table-cell xsl:use-attribute-sets="default-cell-style" border-right="{$wide-border-style}">
			<xsl:apply-templates />
		</fo:table-cell>
	</xsl:template>
		
	<xsl:template match="Cell[contains(' 2 7 11 12 14 ', concat(' ', @column-number, ' ')) and ../@group-number = count(/Table/Rows/Row)]" priority="2">
		<fo:table-cell xsl:use-attribute-sets="last-row-cell-style" border-right="{$wide-border-style}">
			<xsl:apply-templates />
		</fo:table-cell>
	</xsl:template>
	
	<xsl:template match="Cell[contains(' 1 15 ', concat(' ', @column-number, ' '))]" priority="3">
		<fo:table-cell xsl:use-attribute-sets="default-cell-style" text-align="left">
			<xsl:apply-templates />
		</fo:table-cell>
	</xsl:template>
		
	<xsl:template match="Cell[contains(' 1 15 ', concat(' ', @column-number, ' ')) and ../@group-number = count(/Table/Rows/Row)]" priority="3">
