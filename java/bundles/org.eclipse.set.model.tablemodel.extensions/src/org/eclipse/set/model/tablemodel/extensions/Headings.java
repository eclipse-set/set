...
public class Headings {
	 ...
	 *            the headings
	 * 
	 * @return the maximum height of the headings (a max value of zero falls
	 *         back to the {@link ToolboxConfiguration#getDefaultHeight()
	 *         default height})
	 */
	public static float getMaxHeight(
			final Collection<ColumnDescriptor> headings) {
		float result = 0f;
		for (final ColumnDescriptor v : headings) {
			final double h = v.getHeight();
			if (h > result) {
				result = (float) h;
			}
		}
