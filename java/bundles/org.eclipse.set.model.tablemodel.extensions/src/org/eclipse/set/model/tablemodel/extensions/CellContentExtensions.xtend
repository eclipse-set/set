...
class CellContentExtensions {
	...
	static def dispatch String getRichTextValueWithFootnotes(
		...
		val footnoteText = fc.footnotes.map [
			'''*«getFootnoteNumber(content, it.footnote)»'''
	...
	static def <T, U> Iterable<U> formatCompareContent(
		...
		formatCompareContent(
			(content.oldFootnotes.footnotes + content.unchangedFootnotes.footnotes).map[footnote],
			(content.newFootnotes.footnotes + content.unchangedFootnotes.footnotes).map[footnote],
			oldFormatter,
			commonFormatter,
			newFormatter,
			postFormatter,
